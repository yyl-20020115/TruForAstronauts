...
checkpoint = torch.load(model_state_file, map_location=torch.device('cpu'))#新添加
...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')#////////新添加//////////
...
with torch.no_grad():
    for index, (rgb, path) in enumerate(tqdm(testloader)):
        ...
        if os.path.splitext(os.path.basename(output))[1] == '':  # output is a directory
            ...
            filename_out = output
        if filename_out.startswith('\\'):
            filename_out = '..\\output\\' + filename_out
